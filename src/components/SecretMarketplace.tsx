...
export const SecretMarketplace: React.FC<SecretMarketplaceProps> = ({
  ...
  // Chief-only settled toggle state.
  ...
  const toggleSettled = async (o: SecretOrder) => {
    if (settlingId) return;
    setSettlingId(o.id);
    try {
      const next = !o.settled;
      await secretApi.setSettled(o.id, next);
      soundEffects.playSuccessChime();
      onToast(next ? `Marked ${o.fullName}'s item settled.` : `Marked ${o.fullName}'s item unsettled.`);
      await load();
    } catch (err) {
      soundEffects.playError();
      onToast(err instanceof ApiError ? err.message : 'Could not update status');
    } finally {
      setSettlingId(null);
    }
  };
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
              ...
              {loading ? (
                ...
                <div className="space-y-2">
                  {products
                    .filter((p) => isChief || !boughtIds.has(p.id))
                    .map((p) => {
                    ...
                    {buyers.map((b) => (
                      ...
                      >
                        ...
                        <div className="shrink-0 flex items-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => toggleSettled(b)}
                            disabled={settlingId === b.id}
                            title={b.settled ? 'Mark unsettled' : 'Mark settled'}
                            className={`px-2 py-1 rounded-lg text-[9px] font-bold transition-all cursor-pointer disabled:opacity-60 ${
                              b.settled
